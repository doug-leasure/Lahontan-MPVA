model{
  ...
  for (i in 1:npops) {
    ...
    for (t in t_ti[i,1]:t_ti[i,nt[i]]){
      ...
      N[i,t] ~ dpois( max(0, N[i,t-1] + reintro[i,t-1]) * exp(R[i,t]) )
      ...
      r[i,t] <- b0r + b1r*templag[i,t] + b2r*pow(templag[i,t], 2) + b3r*hflowlag[i,t]
    ...
    sigmaR[i] ~ dt(musig, pow(sigsig,-2), 1) T(0,)
  }
  musig ~ dunif(0, 10)
  sigsig ~ dunif(0, 10)
  ...
  for(i in 1:npops){
    for(t in 1:nt[i]){
      ...
      # Yi[i,t] ~ dbin( Psite[i,t],  N[i,t_ti[i,t]] )
      # 
      # ## Yj ~ total fish observed among all passes at a site
      # Yj[i, t, 1:nj[i,t]] ~ dmulti( PIsite[i,t,1:nj[i,t]],  Yi[i,t] )
      # 
      # ## Psite = probability that capture occurred at one of the sites
      # Psite[i,t] <- sum( pisite[i,t,1:nj[i,t]] )
      
      ## n ~ true site abundance
      # n[i,t,1:(nj[i,t]+1)] ~ dmulti( rho[i,t,1:(nj[i,t]+1)] , N[i,t_ti[i,t]] )
      ...
      for(j in 1:nj[i,t]){
        ...
        ## rho ~ probability that an individual from the population occurs at a given site
        psi[i,t,j] ~ dbeta(prop[i,t,j]*tau, (1-prop[i,t,j])*tau) T(1e-10, 1-1e-10)
        
        # ## pisite = probability of being at a given site and being detected in one of the passes
        # pisite[i,t,j] <- rho[i,t,j] * ( 1 - prod(1 - p[i,t,j,1:npasses[i,t,j]]) )
        # 
        # ## PIsite = probability of being at a given site and being detected in one of the passes, given capture occurred at one of the sites
        # PIsite[i,t,j] <- ifelse( Psite[i,t] == 0,
        #                          0,
        #                          pisite[i,t,j] / Psite[i,t] )
      }
      # rho[i,t,nj[i,t]+1] ~ dbeta(prop[i,t,nj[i,t]+1]*taurho, (1-prop[i,t,nj[i,t]+1])*taurho) T(1e-10, 1-1e-10)
    }
  }
  ## taurho ~ sampling precision
  tau ~ dunif(0, 1e6)
  
  ## !! Consider making taurho pop-specific:  taurho[i] ~ dgamma(alpha, beta)
    ...
    for(t in 1:nt[i]){
      for(j in 1:nj[i,t]){
        ...
        for(m in 1:npasses[i,t,j]){
          ...
          p[i,t,j,m] <- ilogit(b0p + b1p*drain[i,t,j]) * exp(delta * (m - 1))
  ...
  b0p ~ dnorm(0, pow(3,-2)) #  dnorm(-0.4, pow(0.7,-2))
  b1p ~ dnorm(0, pow(3,-2))

  ## delta ~ decrease in detection each pass (parameter of negative exponential function)
  delta ~ dnorm(0, pow(10,-2)) I(,0) #  dnorm(-0.4, pow(0.7,-2)) I(,0)
}
